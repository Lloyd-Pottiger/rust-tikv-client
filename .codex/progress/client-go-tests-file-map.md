...
| `client-go/internal/locate/region_request_state_test.go` | partial | stale/replica read 选路与 retry fallback（含 `DataIsNotReady` stale-read wait+retry + retry fallback-to-leader）语义：`src/request/read_routing.rs` + `src/request/plan.rs` 单测 + `tests/integration_tests.rs` `txn_stale_read_snapshot_get`；mocktikv FSM/async reload N/A |
...
| `client-go/rawkv/rawkv_test.go` | partial | 可迁移 raw 语义（CF 透传、batch/scan/delete_range/cas/checksum/sharding、StoreNotMatch->refresh store addr 重试 + store replace 场景的 StoreNotMatch->invalidate->leader 切换重试）由 Rust unit/E2E 覆盖：`src/raw/client.rs` + `src/request/plan.rs` + `tests/integration_tests.rs`；mocktikv liveness/MVCC 相关用例 N/A |
